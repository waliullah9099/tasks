...
describe("Banner Component", () => {
  ...
  it("renders the main heading correctly", () => {
    ...
    expect(heading).toHaveClass('text-4xl md:text-6xl text-gray-800 font-medium mb-4 capitalize');
    expect(heading.textContent.replace(/\s+/g, ' ').trim())
      .toBe('best collection for home decoration');
  });
  ...
  it('renders the banner with correct background image and styling', () => {
    const bannerDiv = document.querySelector('.bg-cover.bg-no-repeat.bg-center');
